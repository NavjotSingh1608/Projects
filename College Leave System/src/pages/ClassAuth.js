...
const ClassAuth = () => {
  ...
  const updateyes = async (id, per) => {
    ...
    const change = { per_class: true, denialreason: "sent to HOD" };
  ...
  return (
    <>
      ...
      />
      <Collapse in={alert2}>
        <Alert severity="error">You have not specified the denial reason</Alert>
        {setTimeout(() => setalert2(false), 2500)}
      </Collapse>
      <Collapse in={alert}>
        <Alert severity="info">
          You have rejected the leave application .{" "}
        </Alert>
              ...
              <BigCard
                ...
                yes={
                  <Button
                    ...
                    onClick={() => {
                      updateyes(users.id, users.per_class);
                    }}
                  >
                    Yes{" "}
                ...
                no={
                  <Button
                    ...
                    sx={{ width: "100px", height: "40px" }}
                    onClick={() => {
                      updateno(users.id, users.per_class);
                    }}
                  >
                    NO <CancelIcon sx={{ color: "white" }}></CancelIcon>
                  </Button>
                }
                denialreason={
      ...
      {cnt ? (
        ...
        <div>
          <Alert severity="info">No Applications Pending </Alert>
        </div>
      )}
    </>
  );
